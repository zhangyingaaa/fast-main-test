...
class Encoder2D(nn.Module):
    def __init__(self, config: TransConfig, is_segmentation=True):
        ...
        self.patch_size = config.patch_size  # patch_size == (patch_h, patch_w)
...
class Decoder2D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, features=[512, 256, 128, 64]):
        ...
        self.decoder_1 = nn.Sequential(
                    ...
                    nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True)  # scale_factor 可以为数值也可以为元组 表示倍数
                )
...
class SETRModel(nn.Module):
    ...
    def forward(self, x):
        _, final_x = self.encoder_2d(x) # _代指原类型 final_x代指reshape后的类型
